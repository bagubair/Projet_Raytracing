...
class Scene:
    ...
    def calcul_reflexion(self, V, N, bounce, point_intersection,obj_int):
        ...
        if bounce <1:
            ...
            reflexion_dir = (((N * ( ((V*-1).dot_product(N))))+ V))
            
            
            reflexion_orig = point_intersection +(N*0.001)  # Point d'intersection légèrement décalé
    ...
    def calcul_refraction(self, V, N, n1, n2, point_intersection,bounce,obj_int):
        ...
        if bounce == 1 :
            if point :
                ...
                if isinstance(obj_int,Sphere) :
                    if point.components()[2] != point_intersection.components()[2]:
                        pass
                        # print("point_int",point_intersection.components())
                        # print("point",point.components())
                        # print("V" ,V.components())
                        # print("N",N.components())
                        # print("refract_dir",refract_dir.components())
                        # print("refract_orig",refract_orig.components())
        if bounce < 1 :
    ...
    def refract(self, V, N, n1, n2):
        ...
        cosT = (1.0 - sinT2) ** 0.5
        # print("cosT",cosT)
        # print((V * n + N * (n * cosI - cosT)).components())
